...
* *It’s very easy to calculate for every programming language* (only very basic lexical/syntax analysis is needed).
On the other hand, estimation of actual possible paths of execution involves analysis of “possible” and “impossible” paths.
...
image::saturation-curve.svg[{image-40-width}]
...
* Mutated versions should fail the tests!
If test indeed fails, it means that the mutation is *killed* (which is good), otherwise it means that the mutation is *survived* (which means that the test has a flaw!)
...
* Look at the *coverage report, not just the metric*.
Find potentially risky fragments and apply techniques such as fuzzing and/or mutation testing to critical code.
* Keep an eye on modern tooling.
Practical tools for C2 and mutation coverage are improving, but there is still more to come.
