...
long taxableIncome = Math.max(0, income - allowance); //max → agrument, 0 → 1
...
== What mutation tests tell us?

* Test case for income below allowance worth adding, as well as tests for boundary values.
* `> → >=` mutations are *equivalent mutations*, as tax value is continuous function, and these are *false positives*.
